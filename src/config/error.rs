...
pub enum ConfigError {
    ...
    #[error(
        ".pithos toolchains.{toolchain}: version `{value}` is not accepted; specify an exact version like `1.85.0`"
    )]
    FloatingVersionRejected { toolchain: String, value: String },

    #[error(
        ".pithos toolchains.{toolchain}: version `{value}` must match `N`, `N.N`, or `N.N.N` (digits only)"
    )]
    InvalidVersion { toolchain: String, value: String },

    #[error(
        ".pithos toolchains.{toolchain}: version must be a quoted string; wrap the value in quotes, e.g. `{toolchain}: \"1.85.0\"`"
    )]
    NonStringVersion { toolchain: String },

    #[error(
        ".pithos toolchains.{toolchain}: expected a version string, got a mapping; use the flat form, e.g. `{toolchain}: \"1.85.0\"` (nested `version:` keys are not supported)"
    )]
    ...
    #[error(
        ".pithos extras.apt: invalid package name `{entry}`; must match `^[a-z0-9][a-z0-9.+-]+$`"
    )]
    ...
    #[error(".pithos pi.{key}: unknown key; valid: {}", ListBackticked(VALID_PI))]
    ...
    #[error(
        ".pithos pi.version: must be a quoted string; wrap the value in quotes, e.g. `version: \"0.75.3\"`"
    )]
    NonStringPiVersion,

    #[error(
        ".pithos pi.version: version `{value}` must match `N`, `N.N`, or `N.N.N` (digits only)"
    )]
    InvalidPiVersion { value: String },

    #[error(
        ".pithos pi.version: version `{value}` is not accepted; specify an exact version like `0.75.3`"
    )]
    ...
    #[error(
        ".pithos pi.extensions.{name}: must be a quoted string with prefix `npm:` or `git:`, e.g. `\"npm:0.10.7\"` or `\"git:https://example.com/repo#v1.0.0\"`"
    )]
    ...
    #[error(
        ".pithos pi.extensions.{name}: npm version `{value}` is not accepted; specify an exact version like `npm:0.10.7`"
    )]
    FloatingExtensionVersionRejected { name: String, value: String },

    #[error(
        ".pithos pi.extensions.{name}: npm version `{value}` must match `N`, `N.N`, or `N.N.N` (digits only)"
    )]
    InvalidExtensionVersion { name: String, value: String },

    #[error(
        ".pithos pi.extensions.{name}: git spec `{value}` must include a ref, e.g. `git:https://example.com/repo#v1.0.0`"
    )]
